...
#Number using each strategy, per question
strategies1
strategies2
strategies3

#Number using each strategy, per question, per format
int1
static1
paper1
...
# example types per question
q1_et
q2_et
q3_et

# example types per format
f
...
strategies_totals <- data.frame("Format"= c(rep("paper", 3), rep("interactive", 3), rep("static", 3)), "Strategy"=  c(rep(c("trial_and_error", "transformation", "analysis"), 3)), "total"= c(paper_te, paper_tr, paper_an, int_te, int_tr, int_an, static_te, static_tr, static_an))

strategies_plot2 <- ggplot(strategies_totals, mapping=aes(x=Strategy, y=total))+geom_bar(stat="identity")+facet_wrap(~Format)+labs(y="Number of Participants", title="Total number of articipants using each strategy in each format, for all questions.", subtitle="Note that this is based off the groups opinion and interpretation of the strategies used by participants.")+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

strategies_plot2

ggsave("Plots/strategy.png", strategies_plot2)
